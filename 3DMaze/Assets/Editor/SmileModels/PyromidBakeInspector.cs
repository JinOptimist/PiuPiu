...
[CustomEditor(typeof(PyromidBakeSettings))]
public class PyromidBakeInspector : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Create"))
        {
            ...
            var shaderGUID = AssetDatabase.FindAssets("PyromidBuilder").FirstOrDefault();
            if (string.IsNullOrEmpty(shaderGUID))
            {
                Debug.LogError("Cannot find compute shader: PyromidBuilder.compute");
            }
            else
            {
                ...
                // Run the baker
                var settings = serializedObject.targetObject as PyromidBakeSettings;
                bool success = PyromidBaker.Run(shader, settings, out var generatedMesh);
